...
module PMS.Infra.Agent.Process.App.Control where
...
import PMS.Infra.Agent.Process.DM.Constant
import PMS.Infra.Agent.Process.DS.Utility
import PMS.Infra.Agent.Process.DS.Core

-- |
--
run :: DM.DomainContext ()
run domDat = do
  hPutStrLn stderr "[INFO] PMS.Infra.Agent.Process.App.Control.run called."
...
runWithLogger (logger, finalizeLogger) appDat domDat = 
  ...
  where
    finalize = do
      hPutStrLn stderr "-----------------------------------------------------------------------------"
      hPutStrLn stderr "[INFO] PMS.Infra.Agent.Process.App.Control.run finalize called."
      finalizeLogger
      hPutStrLn stderr "-----------------------------------------------------------------------------"

    exception e = do
      hPutStrLn stderr "-----------------------------------------------------------------------------"
      hPutStrLn stderr "[ERROR] PMS.Infra.Agent.Process.App.Control.run exception occurred."
    ...
    errorEnd e = do
      hPutStrLn stderr "-----------------------------------------------------------------------------"
      hPutStrLn stderr "[ERROR] PMS.Infra.Agent.Process.App.Control.run end with error."
      hPutStrLn stderr $ show e
      hPutStrLn stderr "-----------------------------------------------------------------------------"
